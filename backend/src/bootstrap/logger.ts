...
const baseOptions: LoggerOptions = {
  ...
  redact: {
    paths: ["password", "passwordHash", "token", "accessToken", "refreshToken", "authorization", "cookie"],
    censor: "[REDACTED]",
  },
  base: isProduction
    ? {
...
export const formatDevLog = (
  ...
  const durationStr = durationMs !== undefined ? `\x1b[90m${durationMs}ms\x1b[0m` : "";
  const pathPadded = (path + "                                        ").slice(0, 36);
