...
export const geminiClient =
	env.GEMINI_API_KEY && !env.GEMINI_API_KEY.includes("placeholder")
	...
	for (const modelName of modelsToTry) {
		try {
			...
			const timeoutPromise = new Promise<never>((_, reject) =>
				setTimeout(
					() => reject(new Error(`Gemini request timed out after 10000ms`)),
					10000,
				),
			);
			...
			logger.warn(
				{ model: modelName, error: err.message },
				`Gemini model ${modelName} returned error/timeout. Trying fallback...`,
			);
		}
	}
	...
	async generateWithSmartFailover(
		...
		for (const provider of order) {
			...
			if (!isProviderAvailable(provider)) {
				...
				failoverTrail.push(
					`${provider.toUpperCase()}: on cooldown (${secsLeft}s remaining — last error: ${h.lastError})`,
				);
				continue;
			}
