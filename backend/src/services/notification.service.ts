...
import { notifications, users, pushSubscriptions } from "../../database/schema";
...
export class NotificationService {
  ...
  static async dispatch(options: DispatchOptions): Promise<void> {
    ...
    if (targetEmail) {
      const emailHtmlBody = template.bodyTemplate(options.data).map(p => `<p>${p}</p>`).join("");
      
      let actionUrl = template.getDefaultActionUrl ? template.getDefaultActionUrl(options.data, options.clientUrl) : undefined;
    ...
    if (!options.emailOnly && options.userId) {
      try {
        const subscriptions = await db.query.pushSubscriptions.findMany({
          where: (subs, { eq, and }) => and(eq(subs.userId, options.userId as string), eq(subs.isActive, true))
        });
