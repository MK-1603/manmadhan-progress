...
import { EmailTemplateBuilder, EmailTemplateOptions } from "./emails/template.builder";
...
class EmailService {
  ...
  public async sendEmail(options: SendEmailOptions): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      ...
      logger.info({ messageId: info.messageId, to: maskEmail(options.to) }, "Email dispatched successfully with enterprise template");
      return { success: true, messageId: info.messageId };
    } catch (error: any) {
      logger.error({ error: error.message, to: maskEmail(options.to) }, "Email dispatch failed");
      return { success: false, error: error.message };
    }
  }

  public async sendInvitationEmail(to: string, token: string, role: string, inviterName: string = "A team member"): Promise<boolean> {
    const actionUrl = `http://localhost:3000/invite/${token}`;
    logger.info(`[InvitationEmail] Sending invitation to ${to} (${role}) via link: ${actionUrl}`);
    ...
    if (!result.success) {
      logger.warn(`[InvitationEmail] Email send status: false for ${to}. Invitation token URL: ${actionUrl}`);
    }
