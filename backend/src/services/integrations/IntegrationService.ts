...
import type { IIntegrationProvider } from "./providers/IIntegrationProvider";
...
export class IntegrationService {
	...
	public static initialize() {
		IntegrationService.registerProvider(new GoogleCalendarProvider());
		IntegrationService.registerProvider(new GitHubProvider());
		IntegrationService.registerProvider(new RSSProvider());
	}

	public static registerProvider(provider: IIntegrationProvider) {
		IntegrationService.providers.set(provider.providerName, provider);
	}

	public static getProvider(providerName: string): IIntegrationProvider {
		const provider = IntegrationService.providers.get(providerName);
		if (!provider) {
			throw new Error(
				`Integration provider '${providerName}' not found or not supported.`,
			);
		}
		return provider;
	}
	...
	public static async connectOAuth(
		...
		const provider = IntegrationService.getProvider(providerName);
		if (!provider.handleCallback) {
			throw new Error(
				`Provider '${providerName}' does not support OAuth callbacks.`,
			);
		}
		...
		const existingAccount = existing.find(
			(a) => a.provider === providerName && a.accountId === accountId,
		);
		...
		if (existingAccount) {
			...
			const [created] = await personalDb
				.select()
				.from(integrationAccounts)
				.where(eq(integrationAccounts.id, newId));
			return created;
		}
	}
	...
	public static async syncIntegration(
		...
		const provider = IntegrationService.getProvider(account.provider);
		...
		if (provider.validateOrRefreshToken) {
			...
			if (!refreshed) {
				...
				throw new Error(
					"Failed to validate or refresh token. Reauthorization required.",
				);
			}
		}
