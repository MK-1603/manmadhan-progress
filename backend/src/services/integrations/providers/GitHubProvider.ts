...
import { integrationAccounts, integrationGithubRepos, integrationGithubIssues } from "../../../../database/schema/personal.schema";
  ...
  public async sync(integrationAccountId: string, userId: string) {
    ...
    for (const repo of repos) {
      ...
      if (existingRepo) {
        ...
        await personalDb.update(integrationGithubRepos).set(repoData).where(eq(integrationGithubRepos.id, dbRepoId));
      } else {
        dbRepoId = uuidv4();
      ...
      try {
        ...
        for (const issue of issues) {
          ...
          const existingIssue = existingIssues.find(i => i.providerIssueId === providerIssueId);
          ...
          const issueData = {
            ...
            providerCreatedAt: issue.created_at ? new Date(issue.created_at) : null,
            providerUpdatedAt: issue.updated_at ? new Date(issue.updated_at) : null,
          };

          if (existingIssue) {
            await personalDb.update(integrationGithubIssues).set(issueData).where(eq(integrationGithubIssues.id, existingIssue.id));
            recordsUpdated++;
          } else {
