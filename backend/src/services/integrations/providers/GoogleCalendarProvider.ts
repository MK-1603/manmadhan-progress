...
import { integrationAccounts, integrationGoogleCalendars, integrationGoogleCalendarEvents } from "../../../../database/schema/personal.schema";
...
export class GoogleCalendarProvider implements IIntegrationProvider {
  ...
  public async validateOrRefreshToken(integrationAccountId: string): Promise<boolean> {
    ...
    oauth2Client.setCredentials({
      ...
      refresh_token: account.refreshToken ? EncryptionService.decrypt(account.refreshToken) : undefined,
    ...
    const isExpired = account.tokenExpiresAt && (account.tokenExpiresAt.getTime() < Date.now() + 300000);
  ...
  public async sync(integrationAccountId: string, userId: string) {
    ...
    for (const cal of calendars) {
      ...
      if (existingCal) {
        ...
        await personalDb.update(integrationGoogleCalendars).set({
      ...
      const currentCal = existingCal || { isSelected: cal.primary ? true : false };
      ...
      do {
        ...
        for (const evt of events) {
          ...
          const existingEvents = await personalDb
            ...
            .where(eq(integrationGoogleCalendarEvents.calendarId, dbCalendarId));
            
          const existingEvent = existingEvents.find(e => e.providerEventId === providerEventId);

          const evtData = {
            title: evt.summary || "Untitled Event",
            description: evt.description,
            startTime: evt.start?.dateTime ? new Date(evt.start.dateTime) : (evt.start?.date ? new Date(evt.start.date) : null),
            endTime: evt.end?.dateTime ? new Date(evt.end.dateTime) : (evt.end?.date ? new Date(evt.end.date) : null),
          ...
          if (existingEvent) {
             await personalDb.update(integrationGoogleCalendarEvents).set(evtData).where(eq(integrationGoogleCalendarEvents.id, existingEvent.id));
             recordsUpdated++;
          } else {
