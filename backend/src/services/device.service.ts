...
export class DeviceService {
  ...
  static async registerDevice(
    ...
    const isMobile = deviceInfo.os.toLowerCase().includes("android") || deviceInfo.os.toLowerCase().includes("ios");
    
    // Get active devices for the user
    const activeSessions = await db
      .select()
      .from(deviceSessions)
      .where(and(eq(deviceSessions.userId, userId), eq(deviceSessions.isRevoked, false)))
      .orderBy(desc(deviceSessions.loginTime));

    const desktopSessions = activeSessions.filter(
      (s) => !(s.os?.toLowerCase().includes("android") || s.os?.toLowerCase().includes("ios"))
    );
    ...
    if (isMobile && mobileSessions.length >= 5) {
      await this.revokeSession(mobileSessions[mobileSessions.length - 1].id);
    } else if (!isMobile && desktopSessions.length >= 5) {
      await this.revokeSession(desktopSessions[desktopSessions.length - 1].id);
    }
