...
const initWorkers = () => {
  ...
  try {
    emailWorker = new Worker(
      "email-tasks",
      async (job: Job) => {
        logger.info({ jobId: job.id }, "Processing background BullMQ email job");
        if (process.env.MOCK_EMAILS === "true") {
          logger.info({ to: maskEmail(job.data.to), subject: job.data.subject, html: job.data.html }, "MOCK EMAIL DISPATCH (MOCK_EMAILS=true)");
        } else {
...
export const queueService = {
  ...
  async addEmailJob(payload: { to: string; subject: string; text?: string; html?: string; descriptions?: string[]; title?: string; actionUrl?: string; actionText?: string; requestDetails?: Record<string, string>; securityNotice?: boolean; expiresIn?: string; otpCode?: string; mode?: "action" | "alert" | "digest" | "informational"; icon?: "shield" | "user-plus" | "check-circle" | "alert-triangle" | "bell" | "mail" | "credit-card" | "briefcase" | "key" }) {
    ...
    if (isRedisConnected) {
      try {
        const job = await emailQueue.add("send-async-email", { ...payload, html });
        return { success: true, mode: "bullmq_redis", jobId: job.id, message: "Job queued cleanly with auto-cleanup enabled" };
      } catch (e) {}
    }
    // Graceful in-memory fallback if Redis is offline
    if (process.env.MOCK_EMAILS === "true") {
      logger.info({ to: maskEmail(payload.to), subject: payload.subject, html: html }, "MOCK EMAIL DISPATCH (MOCK_EMAILS=true, DIRECT FALLBACK)");
    } else {
      setImmediate(() => emailService.sendEmail({ ...payload, html }));
    }
    return { success: true, mode: "direct_fallback", jobId: `fallback_${Date.now()}`, message: "Processed via direct async worker fallback" };
  },

  async addPushJob(payload: { token?: string; topic?: string; title: string; body: string }) {
    if (isRedisConnected) {
      try {
        const job = await pushQueue.add("send-async-push", payload);
        return { success: true, mode: "bullmq_redis", jobId: job.id, message: "Push job queued cleanly with auto-cleanup enabled" };
      } catch (e) {}
    }
    setImmediate(() => firebaseNotificationService.sendPushNotification(payload));
    return { success: true, mode: "direct_fallback", jobId: `fallback_${Date.now()}`, message: "Processed via direct async worker fallback" };
  },
