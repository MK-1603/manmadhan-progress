  ...
  passport.use(
    new GitHubStrategy(
      ...
      (accessToken: string, refreshToken: string, profile: Profile, done: (err: any, user?: any) => void) => {
        authLogger.info({ githubId: profile.id, username: profile.username }, "GitHub OAuth strategy authenticated user");

        const user = {
          id: profile.id,
          name: profile.displayName || profile.username || "GitHub User",
          email: profile.emails?.[0]?.value || `${profile.username}@users.noreply.github.com`,
          avatar: profile.photos?.[0]?.value || `https://avatars.githubusercontent.com/u/${profile.id}`,
          provider: "github",
        };
