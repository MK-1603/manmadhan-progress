...
import { users, workspaces, workspaceMembers, deviceSessions, passwordResets, spaces } from "../../database/schema";
...
authRouter.post("/login", async (req, res) => {
  ...
  if (user.status === "Seeded" || user.status === "Invitation Sent" || user.status === "Created") {
    await OtpService.sendOTP(email);
    await AuditService.logEvent(user.id, "LOGIN_ATTEMPT", "Initiated first-time login (OTP sent)", req.ip || "");
    return res.json({ success: true, nextStep: "OTP_VERIFICATION", email: user.email });
  }
...
authRouter.post("/login/password", async (req, res) => {
  ...
  const userList = await db.select().from(users).where(eq(users.email, email)).limit(1);
  if (userList.length === 0) return res.status(404).json({ success: false, error: "Account not found" });

  const user = userList[0];

  if (!user.passwordHash) {
    return res.status(400).json({ success: false, error: "Password not set up for this account" });
  }

  const isValid = AuthService.verifyPassword(password, user.passwordHash);
  if (!isValid) {
    await AuditService.logEvent(user.id, "LOGIN_FAILED", "Invalid password attempt", req.ip || "");
  ...
  // Check 48 hour logic
  const sessions = await db.select().from(deviceSessions).where(eq(deviceSessions.userId, user.id)).orderBy(desc(deviceSessions.lastActive)).limit(1);
  if (sessions.length === 0 || (Date.now() - new Date(sessions[0].lastActive).getTime() > 48 * 60 * 60 * 1000)) {
    // Requires OTP
    await OtpService.sendOTP(email);
    return res.json({ success: true, nextStep: "OTP_VERIFICATION", email: user.email });
  }
  ...
  await AuditService.logEvent(user.id, "LOGIN_SUCCESS", "Password login successful", req.ip || "");

  const ws = await db.query.workspaceMembers.findFirst({ where: eq(workspaceMembers.userId, user.id) });
  if (ws) {
    socketService.emitToWorkspace(ws.workspaceId, "MEMBER_ACTIVATED", { userId: user.id });
  }
...
authRouter.post("/verify-otp", async (req, res) => {
  ...
  if (!verifyResult.success) {
    await AuditService.logEvent(user.id, "OTP_FAILED", "Failed OTP verification", req.ip || "");
    return res.status(400).json({ success: false, error: verifyResult.message });
  }

  await AuditService.logEvent(user.id, "OTP_VERIFIED", "Successfully verified OTP", req.ip || "");
  ...
  if (user.passwordHash && user.status === "Activated") {
    ...
    const tempToken = jwt.sign({ id: user.id, email: user.email, intent: "setup", step: "PASSWORD_CREATION" }, env.JWT_SECRET, { expiresIn: "30m" });
    return res.json({ success: true, nextStep: "PASSWORD_CREATION", tempToken });
  }
});
...
authRouter.post("/setup/password", verifyTempToken, async (req, res) => {
  ...
  if (setupUser.step !== "PASSWORD_CREATION") {
    return res.status(403).json({ success: false, error: "Invalid setup step progression" });
  }

  await AuthService.savePassword(setupUser.id, password);
  await AuditService.logEvent(setupUser.id, "PASSWORD_CREATED", "User created password", req.ip || "");

  // Next step depends on role
  const tempToken = jwt.sign({ id: setupUser.id, email: setupUser.email, intent: "setup", step: "PROFILE_SETUP" }, env.JWT_SECRET, { expiresIn: "30m" });

  return res.json({ success: true, nextStep: "PROFILE_SETUP", tempToken });
});

authRouter.post("/setup/profile", verifyTempToken, async (req, res) => {
  const { displayName, timezone, language, dateFormat, timeFormat, batchNumber } = req.body;
  const setupUser = (req as any).setupUser;
  ...
  await AuditService.logEvent(setupUser.id, "PROFILE_UPDATED", "User completed profile setup", req.ip || "");

  // If CEO, next is Organization setup. If not, auto-connect and finish.
  const userList = await db.select().from(users).where(eq(users.id, setupUser.id)).limit(1);
  const user = userList[0];

  if (user.role === "CEO") {
    const tempToken = jwt.sign({ id: setupUser.id, email: setupUser.email, intent: "setup", step: "ORGANIZATION_SETUP" }, env.JWT_SECRET, { expiresIn: "30m" });
    return res.json({ success: true, nextStep: "ORGANIZATION_SETUP", tempToken });
  } else {
    // Finish setup for non-CEO
    await db.update(users).set({ status: "Activated" }).where(eq(users.id, user.id));
    ...
    SessionService.issueTokens(res, user, deviceId);
    await AuditService.logEvent(user.id, "LOGIN_SUCCESS", "First time login completed", req.ip || "");
...
authRouter.post("/setup/organization", verifyTempToken, async (req, res) => {
  ...
  await db.insert(workspaces).values({ id: orgId, name: organizationName, type: "org" });
  await db.insert(workspaceMembers).values({ id: randomUUID(), workspaceId: orgId, userId: user.id, role: "CEO" });
  ...
  await AuditService.logEvent(user.id, "ORGANIZATION_CREATED", `Created organization ${organizationName} and community ${communityName || "None"}`, req.ip || "");
  ...
  SessionService.issueTokens(res, user, deviceId);
  await AuditService.logEvent(user.id, "LOGIN_SUCCESS", "CEO Onboarding completed", req.ip || "");
  ...
  authRouter.get("/me", strictAuth, async (req, res) => {
    ...
    const userRecords = await db.select().from(users).where(eq(users.id, authUser.id)).limit(1);
    if (!userRecords.length) return res.status(401).json({ success: false, error: "Unauthorized" });

    const user = userRecords[0];

    // Fetch active workspace and role
    const memberRecords = await db.select().from(workspaceMembers).where(eq(workspaceMembers.userId, user.id));
    const effectiveRole = (memberRecords.length > 0 && memberRecords[0].role) ? memberRecords[0].role : user.role;
    const orgs = memberRecords.length > 0 ? await db.select().from(workspaces).where(eq(workspaces.id, memberRecords[0].workspaceId)).limit(1) : [];
  ...
  authRouter.post("/refresh", async (req, res) => {
    ...
    try {
      ...
      if (!userRecords.length) return res.status(401).json({ success: false, error: "User not found" });

      const newAccessToken = jwt.sign({ id: userRecords[0].id, role: userRecords[0].role }, env.JWT_SECRET, { expiresIn: (env.JWT_ACCESS_EXPIRATION as any) || "15m" });
      res.cookie("auth_token", newAccessToken, { httpOnly: true, secure: env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 15 * 60 * 1000 });
  ...
  authRouter.post("/password/change", strictAuth, async (req, res) => {
    ...
    const isValid = AuthService.verifyPassword(oldPassword, userRecords[0].passwordHash!);
    if (!isValid) return res.status(403).json({ success: false, error: "Incorrect old password" });

    const isReused = await AuthService.isPasswordReused(authUser.id, AuthService.hashPassword(newPassword));
    if (isReused) return res.status(400).json({ success: false, error: "Password was used recently" });
    ...
    await AuditService.logEvent(authUser.id, "PASSWORD_CHANGED", "Password changed successfully", req.ip || "");

    // Email Notification
    await fetch(`http://localhost:${env.PORT || 4000}/api/v1/queue/email-job`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ to: userRecords[0].email, subject: "Password Changed", text: "Your password was recently changed." })
    }).catch(() => { });
  ...
  authRouter.post("/forgot-password", async (req, res) => {
    ...
    return res.json({ success: true, message: "If that email is in our system, a reset link has been sent." });
  });
  ...
  authRouter.get("/reset/verify", async (req, res) => {
    ...
    const resetRecords = await db.select().from(passwordResets)
      .where(and(eq(passwordResets.tokenHash, tokenHash), eq(passwordResets.used, false)))
      .limit(1);
    ...
    const resetSessionJwt = jwt.sign(
      { intent: "password_reset", resetId: resetRecord.id, userId: resetRecord.userId },
      env.JWT_SECRET,
      { expiresIn: "15m" }
    );
  ...
  authRouter.post("/reset-password", async (req, res) => {
    ...
    if (!resetSessionToken || !newPassword) return res.status(400).json({ success: false, error: "Token and new password required" });
    ...
    try {
      ...
      const resetRecords = await db.select().from(passwordResets)
        .where(and(eq(passwordResets.id, decoded.resetId), eq(passwordResets.used, false)))
        .limit(1);
      ...
      const userRecords = await db.select().from(users).where(eq(users.id, resetRecord.userId)).limit(1);
      if (userRecords.length === 0) return res.status(404).json({ success: false, error: "User not found" });
      const user = userRecords[0];
      ...
      const isReused = await AuthService.isPasswordReused(user.id, hashedNewPassword);
      if (isReused) {
        return res.status(400).json({ success: false, error: "This password has been used recently. Please choose a different one." });
      }
      ...
      await AuditService.logEvent(user.id, "PASSWORD_RESET", "Password was reset via enterprise flow", req.ip || "");
  ...
  authRouter.get("/devices", strictAuth, async (req, res) => {
    const authUser = (req as any).user;
    const sessions = await db.select().from(deviceSessions).where(eq(deviceSessions.userId, authUser.id)).orderBy(desc(deviceSessions.lastActive));
  ...
  authRouter.delete("/devices/:deviceId", strictAuth, async (req, res) => {
    ...
    try {
      ...
      const session = await db.select().from(deviceSessions).where(and(
        eq(deviceSessions.id, deviceId as string),
        eq(deviceSessions.userId, authUser.id as string)
      )).limit(1);

      if (session.length === 0) {
        return res.status(404).json({ success: false, error: "Device session not found or unauthorized" });
      }

      await db.delete(deviceSessions).where(eq(deviceSessions.id, deviceId as string));
      await AuditService.logEvent(authUser.id, "DEVICE_REVOKED", `Revoked access for device ${session[0].deviceName || deviceId}`, req.ip || "");
