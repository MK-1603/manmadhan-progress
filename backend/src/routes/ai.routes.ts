...
aiRouter.post("/context", verifyWorkspaceAccess, async (req: Request, res: Response) => {
  try {
    ...
    if (!type || typeof type !== "string") {
      return res.status(400).json({ success: false, error: "Context type is required (e.g., 'document', 'project_brief')." });
    }
...
aiRouter.get("/context/:workspaceId", verifyWorkspaceAccess, async (req: Request, res: Response) => {
  try {
