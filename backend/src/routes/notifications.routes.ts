...
import { pushSubscriptions, notifications } from "../../database/schema";
...
notificationsRouter.patch("/:id/read", async (req: Request, res: Response) => {
  try {
    ...
    const updated = await db.update(notifications)
      ...
      .returning();
    if (!updated[0]) return res.status(404).json({ success: false, error: "Notification not found" });
  ...
  try {
    ...
    if (existing) {
      await db.update(pushSubscriptions)
        .set({ isActive: true, updatedAt: new Date(), p256dh: keys.p256dh, auth: keys.auth })
