...
export type TaskStatus = "Draft" | "Assigned" | "Accepted" | "In Progress" | "Blocked" | "Review" | "Approved" | "Completed" | "Archived";
...
export const isValidTaskTransition = (current: TaskStatus, next: TaskStatus): boolean => {
  if (current === next) return true;
  return TASK_TRANSITIONS[current]?.includes(next) || false;
};

export const isValidProjectTransition = (current: ProjectStatus, next: ProjectStatus): boolean => {
  if (current === next) return true;
