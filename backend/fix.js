...
function walk(dir) {
  fs.readdirSync(dir).forEach(f => {
    ...
    if (fs.statSync(p).isDirectory()) {
      ...
      c = c.replace(/const  = req\.params\.id as string;/g, 'const projectId = req.params.id as string;');
      // Wait, some might have been taskId. I'll just look at the filename or context to restore correctly.
      if (p.includes('tasks')) {
        c = c.replace(/const projectId = req\.params\.id as string;/g, 'const taskId = req.params.id as string;');
        c = c.replace(/const  = req\.params\.id as string;/g, 'const taskId = req.params.id as string;');
      }
      ...
      c = c.replace(/eq\(, req\.params\.id as string\)/g, 'eq(req.params.id as string, req.params.id as string)'); 
...
execSync('git checkout -- src/routes/', { cwd: 'd:/New folder (4)/manmadhan-progress/backend' });
...
function fixProperly(dir) {
  fs.readdirSync(dir).forEach(f => {
    ...
    if (fs.statSync(p).isDirectory()) {
      ...
      let c = fs.readFileSync(p, 'utf8');
      c = c.replace(/const (\w+)Id = req\.params\.id;/g, 'const $1Id = req.params.id as string;');
      c = c.replace(/eq\(([\w\.]+), req\.params\.id\)/g, 'eq($1, req.params.id as string)');
      fs.writeFileSync(p, c);
    }
  });
}
