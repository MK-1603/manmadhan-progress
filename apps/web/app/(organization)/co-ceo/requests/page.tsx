...
export default function CoCeoRequestsPage() {
        ...
        {showLeaveForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
            <PremiumCard className="border-primary/20">
              <h3 className="text-sm font-bold text-foreground mb-4">New Leave Request</h3>
              <div className="grid sm:grid-cols-3 gap-3 mb-3">
                ...
                <div>
                  ...
                  <input type="date" value={leaveStart} onChange={e => setLeaveStart(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/30" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-muted-foreground block mb-1.5">End Date</label>
                  <input type="date" value={leaveEnd} onChange={e => setLeaveEnd(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/30" />
                </div>
              </div>
              <div className="mb-4">
                <label className="text-xs font-semibold text-muted-foreground block mb-1.5">Reason</label>
                <textarea value={leaveReason} onChange={e => setLeaveReason(e.target.value)} rows={2} placeholder="Reason for leave..." className="w-full px-3 py-2.5 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/30 resize-none" />
              </div>
              <div className="flex gap-3">
                <button onClick={handleLeaveSubmit} disabled={submitting || !leaveStart || !leaveEnd || !leaveReason.trim()} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors">
                  {submitting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null} Submit Request
                </button>
                <button onClick={() => setShowLeaveForm(false)} className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent rounded-lg transition-colors">Cancel</button>
              </div>
            </PremiumCard>
          </motion.div>
        )}
            ...
            <div className="space-y-3">
              {!data?.extensions?.length ? (
                ...
                <PremiumCard key={ext.id}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2"><h3 className="text-sm font-semibold text-foreground">{ext.taskTitle || "Task"}</h3><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusBadge(ext.status)}`}>{ext.status}</span></div>
                      <p className="text-xs text-foreground mt-1">{ext.reason}</p>
                      <p className="text-xs text-blue-500 mt-0.5">Proposed: {new Date(ext.proposedDeadline).toLocaleDateString()}</p>
                    </div>
                  </div>
                </PremiumCard>
              ))}
            ...
            <div className="space-y-3">
              {!data?.leaves?.length ? (
                ...
                <PremiumCard key={leave.id}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2"><h3 className="text-sm font-semibold text-foreground">{leave.type} Leave</h3><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusBadge(leave.status)}`}>{leave.status}</span></div>
                      ...
                      {leave.reason && <p className="text-xs text-foreground mt-0.5">{leave.reason}</p>}
                    </div>
                  </div>
                </PremiumCard>
              ))}
