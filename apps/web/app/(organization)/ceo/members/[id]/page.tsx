...
import {
  ArrowLeft, User, Mail, Briefcase, CheckSquare,
  Users, AlertCircle, Loader2, Trophy, ChevronRight,
  PlusCircle, Clock, Shield
} from "lucide-react";
      ...
      <div className="bg-card border border-border rounded-2xl p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-blue-500/10 border-2 border-blue-500/30 text-blue-500 font-bold text-xl flex items-center justify-center shrink-0">
          ...
          <div className="flex items-center gap-2 self-start">
            <Link
              href="/ceo/tasks"
              className="flex items-center gap-1.5 px-3 py-2 bg-card border border-border hover:border-primary rounded-xl text-[12px] font-semibold text-foreground transition-colors"
            >
              <PlusCircle className="w-3.5 h-3.5" /> Assign Task
            </Link>
            <Link
              href="/ceo/projects"
              className="flex items-center gap-1.5 px-3 py-2 bg-primary text-primary-foreground rounded-xl text-[12px] font-semibold transition-colors"
            >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                <div key={s.label} className="bg-background border border-border rounded-xl p-3 text-center">
                  ...
                  <p className="text-[14px] font-bold text-foreground mt-0.5">{s.value}</p>
                </div>
              ))}
            </div>
            <Link
