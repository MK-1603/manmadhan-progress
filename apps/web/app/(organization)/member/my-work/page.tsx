...
import {
  ...
  Play, CheckCircle2, XCircle, ShieldCheck
} from "lucide-react";
...
export default function MemberMyWorkPage() {
  const { user } = useAuth();
  ...
  const handleDeclineTask = async (taskId: string) => {
    ...
    try {
      const workspaceId = localStorage.getItem("workspaceId");
      await apiClient.post(`/org/my-work/tasks/${taskId}/decline`, { workspaceId, reason: "Declined by Member" });
      fetchMyWork();
    } catch (err: any) {
    ...
    <div className="p-4 lg:p-6 max-w-[1400px] mx-auto w-full space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
          <CheckSquare className="w-5 h-5 text-amber-500" /> My Execution Queue
        </h1>
        <p className="text-xs text-muted-foreground mt-0.5">
          Tasks assigned directly to you by your Team Lead or CEO.
        </p>
      </div>
