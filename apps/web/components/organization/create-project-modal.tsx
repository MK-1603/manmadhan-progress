...
export function CreateProjectModal({ isOpen, onClose, onSuccess }: CreateProjectModalProps) {
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/75 backdrop-blur-xs transition-opacity animate-in fade-in duration-200 overflow-x-hidden select-none">
      ...
      <div className="w-full sm:max-w-2xl bg-[#15191F] border border-[#272D36] rounded-t-[20px] sm:rounded-[16px] shadow-2xl flex flex-col max-h-[88vh] sm:max-h-[85vh] overflow-hidden font-sans">
        
        {/* ── MODAL HEADER ── */}
        <div className="px-6 py-4 border-b border-[#272D36] flex items-center justify-between bg-[#111419] shrink-0">
          <div>
            <h2 className="text-[17px] font-extrabold text-[#F2F4F7] tracking-tight">
              Create Project
            </h2>
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#272D36]/60 text-[#8B95A5] hover:text-[#F2F4F7] flex items-center justify-center transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {!createdProjectResult && (
          <div className="px-6 py-3 border-b border-[#272D36] bg-[#111419]/50 shrink-0">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] text-[11.5px] font-bold transition-colors whitespace-nowrap ${
                      isActive
                        ...
                        : isCompleted
                        ? "text-[#F2F4F7] hover:text-[#C9A52A] cursor-pointer"
                        : "text-[#667085]"
                    }`}
                  >
                    <span>{s.label}</span>
        ...
        {createdProjectResult ? (
          ...
          <div className="p-8 flex flex-col items-center justify-center text-center space-y-5 my-auto">
            ...
            <div className="space-y-1 max-w-md">
              ...
              <h3 className="text-[20px] font-extrabold text-[#F2F4F7]">
                {createdProjectResult.name}
              </h3>
              <p className="text-[12.5px] text-[#8B95A5] leading-relaxed">
                Organization project, milestones, document folders, and notifications created.
              </p>
            </div>

            <div className="w-full max-w-md p-4 rounded-[12px] bg-[#111419] border border-[#272D36] text-left space-y-2 text-[12px]">
              <div className="flex items-center justify-between text-[#8B95A5]">
                <span>Assignee:</span>
                <span className="font-bold text-[#F2F4F7]">{assignedToUser?.name} ({assignedToUser?.role})</span>
              </div>
              <div className="flex items-center justify-between text-[#8B95A5]">
                <span>Team Members:</span>
                <span className="font-bold text-[#F2F4F7]">{selectedMembers.length} assigned</span>
              </div>
              <div className="flex items-center justify-between text-[#8B95A5]">
                <span>Priority:</span>
              ...
              {deadline && (
                <div className="flex items-center justify-between text-[#8B95A5]">
                  <span>Deadline:</span>
                  <span className="font-mono font-bold text-[#F2F4F7]">{new Date(deadline).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 pt-2 w-full max-w-md">
              <button
                onClick={onClose}
                className="flex-1 h-[40px] rounded-[8px] bg-[#272D36] text-[#F2F4F7] text-[12.5px] font-bold hover:opacity-90 transition-opacity cursor-pointer"
              >
                Done
              </button>
              <button
          ...
          <div className="p-6 overflow-y-auto space-y-5 flex-1 text-xs">
            ...
            {/* Global Error Banner */}
            {error && (
              <div className="p-3.5 rounded-[10px] bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[12.5px] font-semibold">
                {error}
              </div>
            )}
            ...
            {currentStep === 1 && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-[14px] font-extrabold text-[#F2F4F7]">Project Details</h3>
                  <p className="text-[12px] text-[#8B95A5]">Define the project and what needs to be accomplished.</p>
                </div>

                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[12px] font-bold text-[#F2F4F7]">
                  ...
                  <input
                    ...
                    className={`w-full h-[40px] px-3.5 bg-[#111419] border ${
                      isDuplicateName ? "border-rose-500" : "border-[#272D36]"
                    } rounded-[8px] text-[13px] text-[#F2F4F7] outline-none focus:border-[#C9A52A]`}
                  />
                  <p className="text-[11px] text-[#667085]">Give the project a clear, recognizable name.</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-[#111419] border border-[#272D36] rounded-[8px] text-[13px] text-[#F2F4F7] outline-none focus:border-[#C9A52A] resize-none"
                  />
                  <p className="text-[11px] text-[#667085]">Describe what needs to be accomplished and the expected outcome.</p>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-[#F2F4F7]">Project Type</label>
                    <input
                      type="text"
                      disabled
                      value={projectType}
                      className="w-full h-[38px] px-3 bg-[#111419]/60 border border-[#272D36] rounded-[8px] text-[12px] text-[#8B95A5]"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-[#F2F4F7]">Priority Level</label>
                    <select
                      value={priority}
                      onChange={(e: any) => setPriority(e.target.value)}
                      className="w-full h-[38px] px-3 bg-[#111419] border border-[#272D36] rounded-[8px] text-[12.5px] text-[#F2F4F7] outline-none focus:border-[#C9A52A]"
                    >
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-[14px] font-extrabold text-[#F2F4F7]">Project Assignment</h3>
                  <p className="text-[12px] text-[#8B95A5]">Define ownership and who will execute the work.</p>
                </div>

                {/* Owner */}
                <div className="p-3 rounded-[8px] bg-[#111419] border border-[#272D36] flex items-center justify-between text-[12px]">
                  <span className="text-[#8B95A5]">Project Owner</span>
                  <span className="font-bold text-[#F2F4F7]">{currentUser.name || "Authorized Owner"} ({currentUser.role || "CEO"})</span>
                </div>
                  ...
                  {assignedToUser ? (
                    <div className="flex items-center justify-between p-2.5 bg-[#C9A52A]/10 border border-[#C9A52A]/30 rounded-[8px]">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-[#F2F4F7]">{assignedToUser.name}</span>
                      ...
                      <button
                        ...
                        className="p-1 text-[#8B95A5] hover:text-rose-500 transition-colors cursor-pointer"
                      >
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full h-[40px] pl-9 pr-3.5 bg-[#111419] border border-[#272D36] rounded-[8px] text-[12.5px] text-[#F2F4F7] outline-none focus:border-[#C9A52A]"
                      />

                      {showAssigneeDropdown && (
                        <div className="absolute left-0 right-0 top-full mt-1 max-h-48 overflow-y-auto bg-[#15191F] border border-[#272D36] rounded-[10px] shadow-xl z-50 p-1 divide-y divide-[#272D36]/40">
                          {filteredAssignees.length === 0 ? (
                            <div className="p-3 text-center text-[#8B95A5] text-[12px]">No members found</div>
                          ) : (
                            filteredAssignees.map((u) => (
                              <button
                                ...
                                className="w-full p-2.5 flex items-center justify-between text-left hover:bg-[#C9A52A]/10 rounded-[6px] transition-colors cursor-pointer"
                              >
                                <div>
                                  <div className="font-bold text-[#F2F4F7]">{u.name}</div>
                                  <div className="text-[11px] text-[#8B95A5]">{u.email}</div>
                                </div>
                                <span className="px-2 py-0.5 rounded-full bg-[#272D36] text-[10px] font-bold uppercase text-[#8B95A5]">
                                  {u.role}
                                </span>
                              </button>
                            ))
                ...
                {assignedToUser && assignedToUser.role.toUpperCase() === "MEMBER" && currentUser.role === "CEO" && (
                  <div className="space-y-1.5 p-3 rounded-[8px] bg-amber-500/10 border border-amber-500/20">
                    <label className="text-[12px] font-bold text-amber-400">
                    ...
                    <select
                      ...
                      className="w-full h-[36px] px-3 bg-[#111419] border border-amber-500/30 rounded-[6px] text-[12px] text-[#F2F4F7] outline-none"
                    >
                ...
                <div className="space-y-1.5 relative" ref={memberDropdownRef}>
                  <label className="text-[12px] font-bold text-[#F2F4F7]">Project Members</label>
                  
                  <div className="flex flex-wrap gap-2 mb-1">
                    {selectedMembers.map((m) => (
                      <span key={m.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#272D36] text-[#F2F4F7] text-[11.5px] font-semibold border border-[#272D36]">
                        <span>@{m.name}</span>
                        <button
                          type="button"
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-[38px] pl-9 pr-3.5 bg-[#111419] border border-[#272D36] rounded-[8px] text-[12px] text-[#F2F4F7] outline-none focus:border-[#C9A52A]"
                    />

                    {showMemberDropdown && (
                      <div className="absolute left-0 right-0 top-full mt-1 max-h-48 overflow-y-auto bg-[#15191F] border border-[#272D36] rounded-[10px] shadow-xl z-50 p-1 divide-y divide-[#272D36]/40">
                        {filteredMembers.length === 0 ? (
                          <div className="p-3 text-center text-[#8B95A5] text-[12px]">No eligible members left</div>
                        ) : (
            ...
            {currentStep === 3 && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-[14px] font-extrabold text-[#F2F4F7]">Project Timeline</h3>
                  <p className="text-[12px] text-[#8B95A5]">Set when execution starts and when the project is expected to finish.</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full h-[40px] px-3 bg-[#111419] border border-[#272D36] rounded-[8px] text-[12.5px] text-[#F2F4F7] outline-none focus:border-[#C9A52A]"
                    />
                  </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-[#F2F4F7]">Estimated Hours</label>
                  <input
                    type="number"
                    min="1"
                ...
                {timelinePreviewText && (
                  <div className="p-3 rounded-[8px] bg-[#111419] border border-[#272D36] flex items-center justify-between text-[12px]">
                    <span className="text-[#8B95A5]">Duration Preview</span>
            ...
            {currentStep === 4 && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-[14px] font-extrabold text-[#F2F4F7]">Goals & Deliverables</h3>
                  <p className="text-[12px] text-[#8B95A5]">Define what success looks like for this project.</p>
                </div>

                {/* Goals */}
                <div className="space-y-2">
                  <label className="text-[12px] font-bold text-[#F2F4F7]">Project Goals</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      ...
                      className="flex-1 h-[36px] px-3 bg-[#111419] border border-[#272D36] rounded-[6px] text-[12px] text-[#F2F4F7] outline-none"
                    />
                    <button
                      type="button"
                  ...
                  {goals.map((g, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 rounded-[6px] bg-[#111419] border border-[#272D36] text-[12px]">
                      <span className="text-[#F2F4F7]">{idx + 1}. {g}</span>
                      <button type="button" onClick={() => handleRemoveGoal(idx)} className="text-[#8B95A5] hover:text-rose-400">
                ...
                {/* Deliverables */}
                <div className="space-y-2">
                  <label className="text-[12px] font-bold text-[#F2F4F7]">Deliverables</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                  ...
                  {deliverables.map((d, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 rounded-[6px] bg-[#111419] border border-[#272D36] text-[12px]">
                      <span className="text-[#F2F4F7]">• {d}</span>
                      <button type="button" onClick={() => handleRemoveDeliverable(idx)} className="text-[#8B95A5] hover:text-rose-400">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ))}
                ...
                <div className="space-y-3 pt-2 border-t border-[#272D36]">
                  <div className="flex items-center justify-between">
                    <label className="text-[12px] font-bold text-[#F2F4F7]">Initial Tasks ({initialTasks.length})</label>
                    <button
                      type="button"
                  ...
                  {showAddTaskForm && (
                    <div className="p-3 rounded-[8px] bg-[#111419] border border-[#272D36] space-y-2.5">
                      <input
                        type="text"
                        placeholder="Task Name *"
                        value={taskTitleInput}
                        onChange={(e) => setTaskTitleInput(e.target.value)}
                        className="w-full h-[36px] px-3 bg-[#15191F] border border-[#272D36] rounded-[6px] text-[12px] text-[#F2F4F7]"
                      />
                      <textarea
                        rows={2}
                        ...
                        className="w-full p-2.5 bg-[#15191F] border border-[#272D36] rounded-[6px] text-[12px] text-[#F2F4F7] resize-none"
                      />
                      <div className="grid grid-cols-2 gap-2">
                        <select
                          value={taskAssigneeInput}
                          onChange={(e) => setTaskAssigneeInput(e.target.value)}
                          className="h-[36px] px-2.5 bg-[#15191F] border border-[#272D36] rounded-[6px] text-[12px] text-[#F2F4F7]"
                        >
                  ...
                  {initialTasks.map((t) => (
                    <div key={t.id} className="p-2.5 rounded-[6px] bg-[#111419] border border-[#272D36] flex items-center justify-between text-[12px]">
                      <div>
                        <div className="font-bold text-[#F2F4F7]">{t.title}</div>
                        <div className="text-[11px] text-[#8B95A5]">{t.description || "No description"}</div>
                      </div>
                      <button type="button" onClick={() => handleRemoveInitialTask(t.id)} className="text-[#8B95A5] hover:text-rose-400">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ))}
            ...
            {currentStep === 5 && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-[14px] font-extrabold text-[#F2F4F7]">Review Project</h3>
                  <p className="text-[12px] text-[#8B95A5]">Review the project details before creating it.</p>
                </div>

                {/* Section 1: Details */}
                <div className="p-3.5 rounded-[10px] bg-[#111419] border border-[#272D36] space-y-1.5 text-[12px]">
                  <div className="flex items-center justify-between border-b border-[#272D36] pb-1.5">
                    <span className="font-extrabold text-[#F2F4F7] uppercase tracking-wider text-[11px]">Project Details</span>
                    <button type="button" onClick={() => handleJumpToStep(1)} className="text-[11.5px] font-bold text-[#C9A52A] hover:underline cursor-pointer">
                      Edit
                    </button>
                  </div>
                  <div className="font-extrabold text-[14px] text-[#F2F4F7]">{name}</div>
                  <p className="text-[#8B95A5] leading-relaxed">{description}</p>
                  <div className="flex items-center gap-3 pt-1 text-[11px] text-[#8B95A5]">
                    <span>Type: <strong className="text-[#F2F4F7]">{projectType}</strong></span>
                ...
                <div className="p-3.5 rounded-[10px] bg-[#111419] border border-[#272D36] space-y-1.5 text-[12px]">
                  <div className="flex items-center justify-between border-b border-[#272D36] pb-1.5">
                    <span className="font-extrabold text-[#F2F4F7] uppercase tracking-wider text-[11px]">Assignment</span>
                  ...
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Owner:</span>
                    <span className="font-bold text-[#F2F4F7]">{currentUser.name || "Owner"} ({currentUser.role || "CEO"})</span>
                  </div>
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Assignee:</span>
                    <span className="font-bold text-[#F2F4F7]">{assignedToUser?.name} ({assignedToUser?.role})</span>
                ...
                <div className="p-3.5 rounded-[10px] bg-[#111419] border border-[#272D36] space-y-1.5 text-[12px]">
                  <div className="flex items-center justify-between border-b border-[#272D36] pb-1.5">
                    <span className="font-extrabold text-[#F2F4F7] uppercase tracking-wider text-[11px]">Timeline</span>
                  ...
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Dates:</span>
                    <span className="font-mono font-bold text-[#F2F4F7]">{startDate} → {deadline}</span>
                  </div>
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Estimated Hours:</span>
                    <span className="font-mono font-bold text-[#F2F4F7]">{estimatedHours} hrs</span>
                  </div>
                </div>
                ...
                <div className="p-3.5 rounded-[10px] bg-[#111419] border border-[#272D36] space-y-1.5 text-[12px]">
                  <div className="flex items-center justify-between border-b border-[#272D36] pb-1.5">
                    <span className="font-extrabold text-[#F2F4F7] uppercase tracking-wider text-[11px]">Goals & Deliverables</span>
                  ...
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Goals:</span>
                    <span className="font-bold text-[#F2F4F7]">{goals.length} defined</span>
                  </div>
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Deliverables:</span>
                    <span className="font-bold text-[#F2F4F7]">{deliverables.length} defined</span>
                  </div>
                  <div className="flex items-center justify-between text-[#8B95A5]">
                    <span>Initial Tasks:</span>
                    <span className="font-bold text-[#F2F4F7]">{initialTasks.length} tasks</span>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* ── MODAL FOOTER ── */}
        {!createdProjectResult && (
          <div className="px-6 py-4 border-t border-[#272D36] flex items-center justify-between bg-[#111419] shrink-0">
            <div>
              {currentStep > 1 && (
                <button
                  type="button"
                  onClick={handlePrevStep}
                  className="px-4 h-[38px] rounded-[8px] bg-[#272D36] text-[#F2F4F7] text-[12.5px] font-bold hover:opacity-90 transition-opacity cursor-pointer"
                >
                  Back
                </button>
              )}
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                className="px-4 h-[38px] rounded-[8px] bg-[#272D36]/60 text-[#8B95A5] hover:text-[#F2F4F7] text-[12.5px] font-bold transition-colors cursor-pointer"
              >
                Cancel
              </button>
