...
export function OrganizationSettings({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <div className="min-h-[100dvh] p-4 pb-24 md:p-8 lg:p-10 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-5">{[["Members", "/members"], ["Roles & Permissions", "/members"], ["Notifications", "/inbox"], ["Scoring & Leaderboard", "/leaderboard"], ["Integrations", "/integrations"], ["Security & Audit", "/security"]].map(([label, href]) => <a key={label} href={`${user?.role === "CO-CEO" ? "/co-ceo" : user?.role === "MEMBER" ? "/member" : "/ceo"}${href}`} className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-sm font-semibold hover:bg-muted/50"><span>{label}</span><ExternalLink className="w-4 h-4 text-muted-foreground" /></a>)}</div>
