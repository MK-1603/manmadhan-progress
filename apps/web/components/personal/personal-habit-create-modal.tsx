...
export function PersonalHabitCreateModal({
  isOpen,
  onClose,
  onSave,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiClient.post("/personal/habits", {
        name,
        description,
        category,
        frequency,
      ...
      if (res.data.success) {
        ...
        setError(res.data.error || "Failed to create habit");
      }
    } catch (e: any) {
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)} className="w-full h-9 text-sm px-3 rounded-lg border border-input bg-background focus:ring-2 focus:ring-ring">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={frequency} onChange={(e) => setFrequency(e.target.value)} className="w-full h-9 text-sm px-3 rounded-lg border border-input bg-background focus:ring-2 focus:ring-ring">
                <option>Daily</option>
                <option>Weekly</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Target (Per Frequency)</label>
              <input type="number" min="1" value={target} onChange={(e) => setTarget(e.target.value)} className="w-full h-9 text-sm px-3 rounded-lg border border-input bg-background focus:ring-2 focus:ring-ring" />
            </div>
          </div>
        </div>
