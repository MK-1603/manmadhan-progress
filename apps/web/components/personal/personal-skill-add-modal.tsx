...
export function PersonalSkillAddModal({
  isOpen,
  onClose,
  onSave,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiClient.post("/personal/learning", {
        name,
        description,
        category,
        currentLevel,
      ...
      if (res.data.success) {
        ...
        setError(res.data.error || "Failed to add skill");
      }
    } catch (e: any) {
      ...
      <div className="w-full max-w-lg bg-card border border-border rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Current Level</label>
              <select value={currentLevel} onChange={(e) => setCurrentLevel(e.target.value)} className="w-full h-9 text-sm px-3 rounded-lg border border-input bg-background focus:ring-2 focus:ring-ring">
