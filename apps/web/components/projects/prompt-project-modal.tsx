...
export default function PromptProjectModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/80 p-3 sm:p-4 overflow-y-auto backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col text-foreground transition-colors duration-200">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/40">
          <div>
            <span className="text-[11px] font-bold tracking-wider text-amber-500 uppercase">
              {workspaceType === "ORGANIZATION" ? "Organization Mandate" : "Personal Workspace"}
            </span>
            <h2 className="text-lg font-bold text-foreground mt-0.5">
              {step === "PROMPT" && "Create Project from Prompt"}
              {step === "GENERATING" && "Structuring Execution Mandate..."}
              {step === "REVIEW" && "Review Project Mandate"}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Error Alert */}
        {errorMsg && (
          <div className="mx-6 mt-4 p-3.5 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-600 dark:text-rose-400 text-xs flex items-center gap-2">
